...
def compare_io_files(
    ...
    print_ = functools.partial(printx, flush=True, verbose=True)
    with io_file_operator(src_file_path, mode="r", verbose=verbose) as src_file_op:
        with io_file_operator(trg_file_path, mode="r", verbose=verbose) as trg_file_op:
            if src_file_op is None:
                print_(f"== iodiff: cannot open `{src_file_path}`")
            elif trg_file_op is None:
                print_(f"== iodiff: cannot open `{trg_file_path}`")
            else:
                atol = atol or ATOL
                rtol = rtol or RTOL

                print_("== iodiff: start\n")
                print_(f"   - source file: {src_file_op.f_path}")
                print_(f"   - target file: {trg_file_op.f_path}")
                print_(f"   - atol: {atol:.1E}")
                print_(f"   - rtol: {rtol:.1E}\n")
                ...
                if src_keys_only := (src_keys - {*common_keys}):
                    print_(f"   - fields found only in source file: {','.join(src_keys_only)}")
                if trg_keys_only := (trg_keys - {*common_keys}):
                    print_(f"   - fields found only in target file: {','.join(trg_keys_only)}")
                ...
                if len(common_keys) > 0:
                    ...
                    for key in common_keys:
                        ...
                        if src_field.dtype.kind == "S" and trg_field.dtype.kind == "S":
                            print_(f"   {key:{max_key_len}s}: string")
                            continue

                        if src_field.shape != trg_field.shape:
                            print_(
                                f"   {key:{max_key_len}s}: incompatible shapes {src_field.shape} "
                                f"and {trg_field.shape}"
                            )
                            continue
